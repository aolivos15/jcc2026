...
/* ==========================================================================
   ...
   ========================================================================== */
	...
	$('#modalExpositor').on('show.bs.modal', function (event) {
		const element = $(event.relatedTarget)
		let nombre = element.data('nombre')
		let apellido = element.data('apellido')
		let cargo = element.data('cargo')
		let empresa = element.data('empresa')
		let email = element.data('email')
		let foto = element.data('foto')
		let titulo = element.data('titulo')
		let resumen = element.data('resumen')
		const modal = $(this)
		modal.find('img').attr('src', (foto === '') ? 'images/resource/speaker-1.jpg':foto)
		modal.find('#nombre').text(nombre + ' ' + apellido)
		modal.find('#cargo').html(cargo + ' ' + empresa)
		// modal.find('#cargo').html(cargo + ' - ' + empresa)
		modal.find('#email').html(email)
		modal.find('#titulo').text(titulo)
		modal.find('#resumen').html(resumen)
	});
